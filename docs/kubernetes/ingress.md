...
## Ingress Controller와 일반 Controller의 차이

일반 Controller(Deployment, ReplicaSet 등)는 쿠버네티스 오브젝트만 관리한다. Ingress Controller는 Controller 패턴(Ingress 리소스 감시 → 설정 반영)을 따르면서, 실제 네트워크 트래픽까지 직접 처리한다.

```
Deployment Controller  → 파드 수 조정 (클러스터 오브젝트 관리만)
Ingress Controller     → Ingress 규칙 감시 + nginx로 실제 트래픽 처리
```

reconciliation loop + 데이터 플레인 역할을 동시에 하기 때문에 범위가 더 넓다.
